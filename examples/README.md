...
| ROS 2 导航记忆 | `ros2_nav_demo/` | Docker | 真实仿真导航：探索记忆航点 → 记忆返航（Docker Gazebo/Webots，无需本地 ROS） |
...
ROS 2 真实仿真导航。证明"走过的路不用再走第二次"：
- Session 1: 反应式探索（前进 + 撞墙后退转向），robotmem 记住航点
- Session 2: recall 航点 → 反转 → 原路返航（记忆导航）

支持三种运行方式：
- **Docker Gazebo**（推荐）: `docker compose run gazebo-demo`
- **Docker Webots**: `docker compose run webots-demo`
- **The Construct**: `python3 demo.py 25 --prefix /originbot_1`
